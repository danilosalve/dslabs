...
export class DragDropProductsComponent {
  ...
  constructor(protected poNotificationService: PoNotificationService, protected productService: ProductService) {
  ...
  getValueFromFile(property: string, headersRow: string[], currentyRow: any, type: string): any {
  ...
  getDataRecordsArrayFromCSVFile(csvRecordsArray: any, headersRow: string[]): any[] {
      let csvArr = [];
      ...
      for (let i = 1; i < csvRecordsArray.length; i++) {
          ...
          if (currentyRecord.length > 0) {
              ...
              csvRecord.productId = this.getValueFromFile('code', headersRow, currentyRecord, 'string') || '';
              csvRecord.value = this.getValueFromFile('unitPrice', headersRow, currentyRecord, 'number') || 0;
              csvRecord.quantity = this.getValueFromFile('quantity', headersRow, currentyRecord, 'number') || 0;
              csvRecord.amount = this.getValueFromFile('amountPrice', headersRow, currentyRecord, 'number') || 0;
              csvRecord.discount = this.getValueFromFile('discount', headersRow, currentyRecord, 'number') || 0;
              csvRecord.customerOrderId = this.getValueFromFile('customerOrderId', headersRow, currentyRecord, 'string') || '';
