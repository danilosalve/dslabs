...
export class MoreInformationComponent implements OnInit, OnChanges {
    ...
    onInitForm(): void {
        this.formItem = this.fb.group({
            discount: [
                this.salesItems.discount,
                [Validators.min(0), Validators.max(99.99)]
            ],
                ...
                .subscribe(percentage => {
                    if (this.isAValidDiscountPercentage(percentage)) {
                        ...
                        this.poNotification.error(
                            'Percentual de Desconto invalido'
                        );
                    }
                });
    ...
    onChangeCustomerOrderId(): void {
        this.formItem
            ...
            .subscribe(value => {
              ...
              this.salesItems.customerOrderId = value;
            });
    }
    ...
    calculateDiscountAmount(percentage: number): void {
        ...
        if (percentage > 0) {
            discountValue =
                ((percentage * this.salesItems.value) / 100) *
                this.salesItems.quantity;
            this.formItem
                .get('discountamount')
    ...
    calculateDiscountPercentage(value: number): void {
        ...
        if (value > 0) {
            percentage =
                ((value * 100) / this.salesItems.value) *
                this.salesItems.quantity;
            if (this.isAValidDiscountPercentage(percentage)) {
                this.formItem
                    .get('discount')
    ...
    setItemDiscount(discount: number, percentage: number): void {
        const price =
            this.salesItems.value - discount / this.salesItems.quantity;
        const amount = price * this.salesItems.quantity;

        if (this.salesItems.quantity > 0) {
            this.discountedValue =
                this.currencyPipe.transform(price.toFixed(2), 'BRL') + '';
            ...
            this.formCard
                .get('price')
                ?.setValue(price.toFixed(2), { emitEvent: false });
            this.formCard
                .get('amount')
                ?.setValue(amount.toFixed(2), { emitEvent: false });
        }
    }
}
